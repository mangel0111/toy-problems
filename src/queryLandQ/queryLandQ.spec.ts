...
describe('queryLandQ', () => {
  ...
  describe('processStreamImprovedUsingTries', () => {
    ...
    it('should return the correct output when only Q are provided', () => {
      const output = processStreamImprovedUsingTries(['Q: database']);
      expect(output).toStrictEqual(['ACK: database; ID=1']);
    });
  });
});
